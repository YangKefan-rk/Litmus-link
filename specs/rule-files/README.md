...
Supported combination fields are `category`, `skeleton`, `memory_event`, `attribute`, `tlb`, `cmo`, and `vector`. Use `litmus-link list axes` to print accepted values.
...
Explicit `cases` may carry a free-form `params` object. Current renderers preserve it in `.meta.json`; future backends can use it for addresses, element widths, page layout, or harness controls.
